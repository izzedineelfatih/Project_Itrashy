...
// Proses penukaran voucher jika konfirmasi diterima
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['confirm_exchange'])) {
    $poinRequired = $voucher['points'];
    ...
    if ($poin_terkumpul >= $poinRequired) {
        try {
            ...
            // Generate kode unik
            $unique_code = 'itrashy' . strtoupper(bin2hex(random_bytes(2))); // Menghasilkan 4 karakter acak
            $stmtInsert = $pdo->prepare("INSERT INTO voucher_redeem (user_id, voucher_id, unique_code, created_at) VALUES (?, ?, ?, NOW())");
            $stmtInsert->execute([$user_id, $voucher['id'], $unique_code]);
            ...
            $generated_code = $unique_code; // Simpan kode unik untuk ditampilkan
        } catch (Exception $e) {
...
<head>
    ...
    <meta name="page-title" content="Detail Voucher">
...
<body class="bg-[#f5f6fb] font-sans">
        ...
        <div class="flex-1 flex flex-col min-h-screen lg:ml-0">
            ...
            <div class="flex-1 overflow-y-auto p-7">
                <div class="flex flex-col bg-white max-w-md mx-auto p-5 rounded-lg shadow-lg">
                    ...
                    <button id="tukar-btn" class="w-full bg-blue-500 text-white mt-8 py-2.5 rounded-full hover:bg-blue-600 transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500">Tukar</button>
                </div>
            </div>
            ...
            <div id="confirm-modal" class="fixed inset-0 bg-black bg-opacity-70 hidden flex justify-center items-center z-50">
                <div class="bg-white p-5 rounded-lg shadow-lg w-96">
                    ...
                    <p class="text-center">Yakin ingin menukarkan poin untuk voucher <strong><?= htmlspecialchars($voucher['title']); ?></strong>?</p>
            ...
            <?php if (isset($penukaran_berhasil)): ?>
                <?php if ($penukaran_berhasil): ?>
                    <div id="success-modal" class="fixed inset-0 bg-black bg-opacity-70 flex justify-center items-center z-50">
                        <div class="bg-white p-5 rounded-lg shadow-lg w-96">
                            <h2 class="text-lg font-bold mb-4">Penukaran Voucher Berhasil</h2>
                            <p>Kode unik Anda: <strong id="unique-code"><?= htmlspecialchars($generated_code); ?></strong></p>
                ...
                <?php else: ?>
                    <div id="error-modal" class="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center">
                        <div class="bg-white p-5 rounded-lg shadow-lg w-96">
                            ...
                            <p>Maaf, poin Anda tidak cukup untuk melakukan penukaran voucher ini.</p>
                            <div class="flex justify-center mt-4">
    ...
    <script>
        ...
        confirmBtn.addEventListener('click', function () {
            ...
            form.method = 'POST';
            form.innerHTML = '<input type="hidden" name="confirm_exchange" value="1">';
