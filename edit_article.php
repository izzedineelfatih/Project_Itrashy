...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['id'])) {
        // Update artikel jika ID disediakan
        $stmt = $pdo->prepare("UPDATE articles SET title = ?, content = ?, image_url = ? WHERE id = ?");
        $stmt->execute([$title, $content, $imageUrl, $_POST['id']]);
    } else {
        // Simpan artikel baru jika tidak ada ID
...
// Ambil data artikel untuk form edit jika ada ID artikel dalam URL
$article = null;
if (isset($_GET['id'])) {
...
<head>
    ...
    <title><?= isset($article) ? 'Edit' : 'Add' ?> Article</title>
    ...
    <div class="container mx-auto py-10">
        ...
        <form method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                ...
                <?php if (isset($article['image_url']) && $article['image_url']): ?>
                    <small class="text-gray-500">Current image: <?= htmlspecialchars($article['image_url']) ?></small>
                <?php endif; ?>
            </div>
